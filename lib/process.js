...
const processedArtPath = __dirname.split('\\').slice(0, -1).join('/') + '/src/data/art.js';
...
const rawImages = fs.readdirSync(rawImageDir).filter(image => isFormat(image, ['jpg', 'png']));
const rawCloseups = fs.readdirSync(rawCloseupsDir).filter(image => isFormat(image, ['jpg', 'png']));
  ...
  .then(rawArt => {
    ...
    const processedArt = liveArt.map(artwork => {
      return {
        ...
        buyOgCaption: artwork.buyOgCaption,
        buyOgSmallprint: artwork.buyOgSmallprint,
        buyPrtCaption: artwork.buyPrtCaption,
