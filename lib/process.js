...
//   __dirname.split("\\").slice(0, -1).join("/") + "/src/data/index.ts";
// for mac
const processedArtPath =
  __dirname.split("/").slice(0, -1).join("/") + "/src/data/index.ts";
  ...
  .then((rawArt) => {
    ...
    const processedArt = liveArt.map((artwork) => {
      return {
        ...
        slug: artwork.slug,
        date: artwork.date,
        showcase: Boolean(artwork.showcase),
        spotlight: Boolean(artwork.spotlight),
        closeups: Boolean(artwork.closeups),
...
export const ART = ${JSON.stringify(processedArt)};
`;
